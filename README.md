...
- **The eight survivors are sealed in exploration suits, drawn at eight heads.** `ss.char.dot`…`ss.char.hex` began as the game's eight geometric shells — a disc, a triangle, a square — and are now eight figures in true human proportion, no skin showing anywhere: dark suit fabric under a chest shell, a hard helmet with a black visor, sealed gloves and boots, and the dead pheromone core socketed in the chest where the hive's would be lit. The hive reads scent, and a body wrapped this tight gives it nothing. Limbs come from four hinged segments — `ss.lib.thigh`, `ss.lib.shin`, `ss.lib.upperarm`, `ss.lib.forearm` — each pivoting at its own joint, so a `walk` bends at the knee and the elbow: the shin and forearm carry `x`/`y` tracks that follow the joint above them through the stride. The palette is one dark slate-and-steel suit across all eight, and the colour each survivor used to *be* survives only as a stripe on the chest and a line in the visor. What tells them apart is worn on the body, not held: Dot's two dead feelers grown to twice any other's, arcing back from the crown and hanging to the waist; Tri's swept crest, calf fins and the spit nozzle plumbed through the helmet; Blok's hardsuit bulk, two-tier pauldrons and a pack of cold-fused spore canisters; Hourglass's waist drawn to nothing, banded abdomen tassets, wing-membrane cape and the tether drum on the hip; Prism's faceted chest, the tymbal in the shell that chirps a ring in the idle, a visor that runs the whole way round, sensor vanes on the back; Egg's incubator shell strapped under a cocoon wrap, a cracked dome, bound mandibles racked behind the shoulder; Donut's hollow ring port open through the chest, quills down the spine, a cowl, blades loaded flat on the forearm; Hex's long coat worked with cells, the brood pack with grubs still on it, a feed line and a sensor mast. Each stands a little differently and each carries an `idle` (what the game bakes today) and a `walk`, tuned with `scripts/sheet.ts`; the readability lint holds all eight above the floor at a deliberately dark 2.5 to 2.8.
...
- ~~Part libraries beyond `lib.face`~~ → hinged limb segments (`ss.lib.thigh` / `shin` / `upperarm` / `forearm`) shipped with the survivors; still to come: crowns, telegraph markers, named particle-emitter presets
