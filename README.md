...
- **The eight survivors are small people.** `ss.char.dot`…`ss.char.hex` began as the game's eight geometric shells — a disc, a triangle, a square — and are now eight kids of the hive's brood, drawn two heads tall: a big round head in `$skin` with the `ss.lib.face` on it (two big eyes with a glint, blush, a mouth), bright hair in the colour each one used to *be*, a little outfit in the same colour, stubby limbs from `ss.lib.leg` / `ss.lib.arm` (hinged at the joint, so a `rot` track swings them the way a limb swings), and the two things that make them hive: the dead organ worn as a brooch on the chest, and two bobble-tipped feelers standing out of the hair. The hive has no eyes because it reads by scent; a survivor whose organ has gone dead has nothing left but looking, and this is what grew. The face carries an expression per survivor as a variant of one document — Tri's puffed cheek, Blok's brows, Hourglass's lidded smirk, Prism singing, Egg's pacifier, Donut's wink, Hex's worry — and the insect is worn as accessory: Hourglass's folded wings and wasp bands, Prism's cicada wings and the chirp ring leaving its mouth, Blok's closed elytra, Hex's brood vent carried as a pack with grubs still on it, Egg's eggshell cap, Donut's swim ring grown over the wound. The weapon each one runs on is in its hand or on its hip, composed from the projectile document the game already fires. Each carries an `idle` (what the game bakes today: a bounce, a blink, the feelers sweeping) and a `walk`, tuned with `scripts/sheet.ts`.
