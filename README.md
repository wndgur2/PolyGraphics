...
- **A weapon icon's rim is that weapon's own colour.** Lash `$husk` (bone), Glob `$bile` (acid), Mandibles `$chitin` (jaw amber), Pod `$soul` (fuse light), Broodling `$blood` (vent), Chirp `$frost`, Head `$timber`, Wing `$arcane`. The rims had drifted — three documents claimed to keep a hand-drawn original's colour, and two of those originals no longer existed — so each now takes a colour that is actually in the art it wraps. The arsenal is deliberately four shades of one material, which fights this: Lash, Mandibles, Pod and Head are all bone or chitin or timber, and a rim drawn from the dominant colour of each would put four slots within ΔE2000 13 of one another. So three of them take the part of their art that nothing else owns rather than the part there is most of — the jaw's amber over the husk it is mostly drawn in, the pod's one cold fuse light over its four browns, the head's unlightened timber, which is the only ramp of it that clears chitin. The worst pair is 20.2, Mandibles against Head, and it is the arsenal's own palette that sets that ceiling. `$arcane` on the Wing is ahead of its art on purpose: that weapon is being redrawn as a grenade thorn and going purple. `evolved` overrides all eight with `$gold`, the one rim that means a state rather than a weapon.
