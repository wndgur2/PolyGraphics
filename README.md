...
- **The eight survivors are people of the brood, drawn the way the world is drawn.** `ss.char.dot`…`ss.char.hex` began as the game's eight geometric shells — a disc, a triangle, a square — and are now eight figures at five heads tall, lit from the upper left like everything else on the field: a head in `$skin` wearing `ss.lib.face` (brows, narrowed eyes with a glint, the shadow of a nose, a closed mouth, and an expression per survivor as a variant), limbs from `ss.lib.leg` / `ss.lib.arm` (hinged at the joint, with `plate` variants for the armoured ones), and gear built up in planes — base, highlight, shade, a ridge, a trim — from the same tokens the hive is drawn in: steel and slate, leather, bone, chitin. The colour each one used to *be* is now its cloak, its sash, its trim and its hair. What keeps them hive is worn, not lit: the dead organ set in the breastplate where the hive's would glow, and two dead feelers rising from a circlet, a helm or a hood and streaming back like plumes. The old shape survives as a signature — Tri's hood comes to its point, Blok still cuts a block, Hourglass is pinched to nothing at the waist, Prism's breastplate is two facets on a ridge, Egg wears half its shell as a helm, Donut wears the ring-plate with the hole open through it, Hex's robe is worked with cells. Each is a class before it is a colour: a lasher with a mandible on a cord, a hooded spitter with a full gland under the jaw, a warden in bronze plate with a shield cut from a Soldier's back, a wasp knight with a flail and a wing-cape, a cicada singer with a crystal breastplate and a tymbal at the hip, an unhatched squire with bound mandibles slung on the back, a masked spine-thrower fanning wing blades, a robed broodmother with a vent on her back and grubs still on it. Each carries an `idle` (what the game bakes today) and a `walk`, tuned with `scripts/sheet.ts`; the readability lint holds every one of them above the floor.
