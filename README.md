...
- **The Spider Hand's rim is `$cobalt`, and an evolution passive wears its weapon's rim.** The Spider Hand used to share the wand's `$bile` (ΔE 0); `$cobalt` (#5383c6) is the arms' `$steel` with the grey taken out, in the one stretch of hue no other rim is near — its nearest are `$frost` at ΔE 22.4 and `$arcane` at 22.9. Violet was tried first for Widow's Grip and sat 8.5 from `$arcane`. The eight passives an evolution recipe needs (`ss.icon.cooldown`, `amount`, `speed`, `area`, `might`, `duration`, `maxhp`, `luck`) take their weapon's rim, so a recipe's two halves read as a pair on the slot bar; only the tile changes, and each glyph keeps its own colour.
- **The home screen is authored here too.** `ss.app.icon` is the game's installed icon: the character a new player is given — `use: ss.char.arin`, so it is the character they are actually handed, redrawn when that one is — pale and silent in the middle of a frame washed pink by everyone else's scent. Its `maskable` variant pulls the body inside a round mask's safe zone, because a launcher crops what it likes and a feeler is the first thing a circle takes off. It is the one asset that leaves as a *plate* rather than as IR: `npm run png -- --only ss.app.icon --size 512` (and 192, and 180), baked into the consumer's `public/icons/`, since a web app manifest wants files at sizes it names.
